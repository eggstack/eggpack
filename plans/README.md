...
Eggpack begins from proven distribution-contract work retained in `eggstack/eggup` as the unpublished, frozen `eggup-dist` predecessor. Contract M001 imported the schema-v1 parsing/expansion behavior proven by Eggup distribution M001/M002. Contract M002 is the final producer-contract migration gate: it MUST port and independently qualify the closed Eggup distribution M003 conformance surface before the already-registered Eggup distribution M004 retires the duplicate crate. Eggup remains the source of truth for consumer-side deployment semantics; Eggpack is the source of truth for producer-side distribution semantics, and producer contract/conformance evolution occurs only here after cutover.
